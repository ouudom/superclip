...
def _assemblyai_speech_models(speech_model: str) -> List[str]:
    normalized = (speech_model or "universal").strip().lower()
    aliases = {
        "nano": ["universal-2"],
        "fast": ["universal-2"],
        "best": ["universal-3-pro", "universal-2"],
        "universal": ["universal-3-pro", "universal-2"],
    ...
    requested_models = [
        model.strip()
        for model in re.split(r"[, ]+", normalized)
        if model.strip()
    ]
    valid_models = [
        model
        for model in requested_models
        if model in {"universal-3-pro", "universal-2"}
    ]
    return valid_models or ["universal-3-pro", "universal-2"]
...
def get_video_transcript(video_path: Path, speech_model: str = "universal") -> str:
    ...
    speech_models = _assemblyai_speech_models(speech_model)
